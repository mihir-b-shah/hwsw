...
INP_IR_FILE=$CHAMPSIM_DIR/spec/$1
NAME_BASE=$(basename ${INP_IR_FILE%.*})

OUT_IR_FILE=$CHAMPSIM_DIR/info/$NAME_BASE.ll

CURR_DIR=$(pwd)
cd $INFGEN_DIR/build/ && cmake -DLT_LLVM_INSTALL_DIR=$LLVM_ROOT .. && make && cd $CURR_DIR
run_pass AddDbg $INP_IR_FILE $OUT_IR_FILE

OBJ_FILE=$NAME_BASE.o
EXE_FILE=$CHAMPSIM_DIR/info/$NAME_BASE

$LLC $OUT_IR_FILE -O0 -filetype=obj   \
  -relocation-model=pic               \
  --force-dwarf-frame-section         \
  --frame-pointer=all -o $OBJ_FILE    \
  --debug-entry-values

g++ -lm -O0 $OBJ_FILE -o $EXE_FILE
