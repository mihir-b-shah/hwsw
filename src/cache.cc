		...
		if(PQ.entry[index].pf_origin_level < fill_level)
		  {
		    if (cache_type == IS_L1D)
		      ...
		      {
			cpu = prefetch_cpu;
			PQ.entry[index].pf_metadata = llc_prefetcher_operate(block[set][way].address<<LOG2_BLOCK_SIZE, PQ.entry[index].ip, 1, PREFETCH, PQ.entry[index].pf_metadata, PQ.entry[index].instr_id);
			cpu = 0;
		      }
		  }
		...
		if(mshr_index == -2)
		  {
			...
			if (lower_level->get_occupancy(1, PQ.entry[index].address) == lower_level->get_size(1, PQ.entry[index].address))
			  ...
			  if(PQ.entry[index].pf_origin_level < fill_level)
				...
				{
				  cpu = prefetch_cpu;
				  PQ.entry[index].pf_metadata = llc_prefetcher_operate(PQ.entry[index].address<<LOG2_BLOCK_SIZE, PQ.entry[index].ip, 0, PREFETCH, PQ.entry[index].pf_metadata, PQ.entry[index].instr_id);
				  cpu = 0;
				}
			    }
